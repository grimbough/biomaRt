...
\subsection{Task 4: Select all Affymetrix identifiers on the hgu133plus2 chip and Ensembl gene identifiers for genes located on chromosome 16 between basepair 1100000 and 1250000.}
...
\subsection{Task 5: Retrieve all entrezgene identifiers and HUGO gene symbols of genes which have a "MAP kinase activity" GO term associated with it.}
The GO identifier for MAP kinase activity is GO:0004707.  In our query we will use go as filter and entrezgene and hgnc as attributes.  Here's the query:
\begin{scriptsize}
<<>>=
getBM(c('entrezgene','hgnc'), filters='go', values='GO:0004707', mart=ensembl)
@ 
\end{scriptsize}

\subsection{Task 6:  Given a set of EntrezGene identifiers, retrieve 100bp upstream promoter sequences}
...
\subsection{Task 7: Retrieve all 5' UTR sequences of all genes that are located on chromosome 3 between the positions 185514033 and 185535839}
...
\subsection{Task 8:  Retrieve protein sequences for a given list of EntrezGene identifiers}
...
\subsection{Task 9:  Retrieve known SNPs located on the human chromosome 8 between positions 148350 and 148612}
...
\subsection{Task 10:  Given the human gene TP53, retrieve the human chromosomal location of this gene and also retrieve the chromosomal location and RefSeq id of it's homolog in mouse. }
...
getLDS(attributes = c("hgnc","chromosome_name", "start_position"),
       filters = "hgnc", values = "TP53",mart = human,
...
\subsection{Using a BioMart other than Ensembl}
