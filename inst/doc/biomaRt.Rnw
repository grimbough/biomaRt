...
An object of the \Robject{martTable} class is the output of the Ensembl specific functions and has the following slots:
\begin{itemize}
\item \Robject{id}: stores the id used for querying
\item \Robject{table}: is a list of vectors storing the retrieved data
\end{itemize}

\section{BioMart API functions}
...
<<>>=
getBM(attributes=c("external_gene_id","chr_name","band"),filter="affy_hg_u95av2",values=c("1939_at","2082_s_at","1454_at"), mart=mart)
martDisconnect(mart)
@
