...
\begin{itemize}
\item \Robject{ensembl}: stores the RMySQLConnection to Ensembl
\item \Robject{vega}: stores the RMySQLConnection to VEGA
\item \Robject{sequence}: stores the RMySQLConnection to sequence mart
\item \Robject{snp}: stores the RMySQLConnection to snp mart
\item \Robject{arrayToSpecies}: Stores mapping from affy arrays to species
\item \Robject{arrayToEnsembl}: Stores mapping from affy arrays to Ensembl tables
\end{itemize}
...
Currently the \Rfunction{getGene} function takes identifiers from Entrez-Gene, Affymetrix, RefSeq and embl.  Besides the id argument, this function also has a species, array and type argument.\\
...
getGene(id = 672, type = "entrezgene", species = "homo_sapiens", mart = mart)
@
...
Currently the \Rfunction{getGO} function takes identifiers from Entrez-Gene, Affymetrix, RefSeq and embl.  Besides the id argument, this function also has a species, array and type argument.\\
The id argument is either a vector of identifiers or a single identifier to be annotated.\\
...
getGO(id = 672, type = "entrezgene", species = "homo_sapiens", mart = mart)
@
...
Currently the \Rfunction{getOMIM} function takes identifiers from entrezgene, affy, RefSeq and embl.  Besides the id argument, this function also has an array ,type and mart argument.\\
The id argument is either a vector of identifiers or a single identifier to be annotated.\\
...
getSNP(chromosome = 8, start = 148350, end = 148612, species ="homo_sapiens", mart = mart)
...
<<>>=
getHomolog(id = 1:20, fromspecies = 'homo_sapiens', tospecies = 'mus_musculus', fromtype = 'entrezgene', totype = 'refseq', mart = mart)
@
