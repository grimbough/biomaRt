...
The function \textit{getGene} uses a vector of query ids to look up the name, description and chromosomal information of the corresponding gene. The type of identifier should be specified with the \textit{type} argument (this can have values like: entrezgene, refseq\_dna, unigene,affy\_hg\_u133\_plus\_2, etc...).  Possible values for the type argument can be retrieved with the listFilters function. The \textit{mart} argument should be used to specify which \textit{Mart} object (which we generated above) to use.
\begin{scriptsize}
<<>>=
...
Identical to the \textit{getGene} function, \textit{getGO} takes the \textit{type} and \textit{mart} arguments.
