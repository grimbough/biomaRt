...
% \VignetteDepends{annotate, hgu95av2, biomaRt}
...
<<>>=
library("biomaRt")
mart <- useMart("ensembl", "hsapiens_gene_ensembl", mysql = TRUE)
@ 
...
Something like adf\_entrezgene
is a database table name, and it isn't all that 
informative, so we can try to get more information by extracting the description
for these database table names. We can also get the location of the attribute in
the attributes list so we don't have to write them all out.

<<>>=
cbind(sapply(mget(attrbuts[grep("swiss",attrbuts)], mart@attributes), 
             function(x) x$description), grep("swiss",attrbuts))
cbind(sapply(mget(attrbuts[grep("entrez",attrbuts)], mart@attributes), 
             function(x) x$description), grep("entrez",attrbuts))
@ 
...
<<>>=
genelist <- getBM(attributes = listAttributes(mart)[c(16,165,392,390,348)],
...
<<>>=
attrbuts[grep("description", attrbuts)]
attrbuts[grep("symbol", attrbuts)]
attrbuts[grep("go", attrbuts)]
attrbuts[grep("^chrom", attrbuts)]
@ 


We want "description", "hgnc\_symbol", "go\_description" and "chrom\_name"

<<>>=
annotlist <- getBM(attributes = listAttributes(mart)[c(116, 199, 196, 81)],
                   filter = "affy_hg_u95av2", values = igenes, mart = mart,
                  output = "list",na.value = "&nbsp;")
@ 
...
<<echo=false>>=
lapply(lapply(annotlist, function(x) x[5:10]), function(y) lapply(y, strwrap))
@ 
...
tstat <- round(esApply(sample.exprSet[igenes,1:10], 1, 
...
<<>>=
table.head <- c("Affy ID", "Entrez Gene", "SwissProt", "UniGene", "RefSeq",
                "Name","Symbol","GO Term", "Chromosome",
                "t-statistic","p-value","Fold change", 
                sampleNames(sample.exprSet)[1:10])
repository  <- list("affy","en","sp","ug","gb")
