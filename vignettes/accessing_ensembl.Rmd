...
Accessing the data available in Ensembl is by far most frequent use of the `r Biocpkg("biomaRt")` package.  With that in mind `r Biocpkg("biomaRt")` provides a number of functions that are tailored to work specifically with the BioMart instances provided by Ensembl.  This vignette details this Ensembl specific functionailty and provides a number of example usecases that can be used as the basis for specifying your own queries.
...
If we print the current `ensembl` object, we can see that the `r ensembl@biomart` database ^[this is how Ensembl name the database on their server] has been selected, but that no dataset has been choosen.
...
*Note: Unfortunately there is not BioMart interface to the Ensembl Bacteria data.  The number of bacterial genomes is in the tens of thousands, and BioMart does not perform well when providing data on that scale.*
